# %%
"""Train Multi-Horizon Baseline and Direct XGBoost models using quarterly splits.
...
6. Saves training metadata and results to models/multi_horizon_quarterly/
...
print("="*70)
print("MULTI-HORIZON QUARTERLY MODEL TRAINING")
print("="*70)
...
for horizon in range(1, NUM_HORIZONS + 1):
    ...
    # ----------------------------------------------------------------
    # 1. BASELINE MODEL (Persistence) - Uses load value from (horizon) periods ago
    # ----------------------------------------------------------------
    print(f"\n[H{horizon}] Training Baseline (Persistence) Model...")

    # For multi-horizon persistence, we predict load[i] using load[i-h]
    # Need to get the original load data (before any shifts) and create baseline predictions
    # Combine all quarters' load data
    all_quarters_load = pd.concat([quarter['load'] for quarter in quarters], axis=0)

    # For each test index, predict using load from h periods ago
    y_pred_baseline_list = []
    for test_idx in y_test_combined.index:
        # Calculate the index h periods ago
        try:
            # Find position of test_idx in all_quarters_load
            pos = all_quarters_load.index.get_loc(test_idx)
            if pos >= horizon:
                # Use load from h periods ago
                baseline_pred = all_quarters_load.iloc[pos - horizon]
            else:
                # If not enough history, use the earliest available value
                baseline_pred = all_quarters_load.iloc[0]
            y_pred_baseline_list.append(baseline_pred)
        except KeyError:
            # If index not found, use mean as fallback
            y_pred_baseline_list.append(y_train_combined.mean())

    y_pred_baseline = np.array(y_pred_baseline_list)

    rmse_baseline = np.sqrt(mean_squared_error(y_test_combined, y_pred_baseline))
    mae_baseline = mean_absolute_error(y_test_combined, y_pred_baseline)
    r2_baseline = r2_score(y_test_combined, y_pred_baseline)
    ...
    horizon_models['baseline'].append(None)  # Baseline doesn't need a model object
...
for q_idx, (test_datasets_q, qinfo) in enumerate(zip(all_test_datasets, quarter_split_info)):
    ...
    for horizon in range(1, NUM_HORIZONS + 1):
        ...
        # Baseline predictions for this quarter and horizon
        # Predict load[i] using load[i-h]
        quarter_data = quarters[q_idx]
        y_pred_baseline_q_list = []
        for test_idx in y_test_q.index:
            try:
                pos = quarter_data.index.get_loc(test_idx)
                if pos >= horizon:
                    baseline_pred = quarter_data['load'].iloc[pos - horizon]
                else:
                    baseline_pred = quarter_data['load'].iloc[0]
                y_pred_baseline_q_list.append(baseline_pred)
            except KeyError:
                y_pred_baseline_q_list.append(y_test_q.mean())

        y_pred_baseline_q = np.array(y_pred_baseline_q_list)
    ...
    print(f"  Horizon 1 (15min) - Baseline RMSE: {quarter_horizon_results[0]['metrics']['baseline']['rmse']:.4f}, "
          f"Direct XGB RMSE: {quarter_horizon_results[0]['metrics']['direct_xgb']['rmse']:.4f}")
    print(f"  Horizon {NUM_HORIZONS} ({NUM_HORIZONS*15}min) - Baseline RMSE: {quarter_horizon_results[-1]['metrics']['baseline']['rmse']:.4f}, "
...
print("\n" + "="*70)
print("SUMMARY: COMPARISON ACROSS HORIZONS (Direct XGB)")
print("="*70)
print(f"\n{'Horizon':<10} {'Minutes':<10} {'RMSE':<12} {'MAE':<12} {'R²':<12}")
print("-"*60)

for h_result in horizon_results:
    h = h_result['horizon']
    h_min = h_result['horizon_minutes']
    m = h_result['metrics']['direct_xgb']
    print(f"{h:<10} {h_min:<10} {m['rmse']:<12.4f} {m['mae']:<12.4f} {m['r2']:<12.4f}")
