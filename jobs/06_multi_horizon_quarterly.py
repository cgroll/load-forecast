...
for horizon, X_all, y_all, split_type_all in all_horizon_datasets:
    ...
    # Get the load data and create baseline predictions
    y_pred_baseline_list = []
    for test_idx in y_test.index:
        try:
            # Find position of test_idx in all_data
            pos = all_data.index.get_loc(test_idx)
            if pos >= horizon:
                # Use load from h periods ago
                baseline_pred = all_data[TARGET_COL].iloc[pos - horizon]
            else:
                # If not enough history, use the earliest available value
                baseline_pred = all_data[TARGET_COL].iloc[0]
            y_pred_baseline_list.append(baseline_pred)
        except KeyError:
            # If index not found, use mean as fallback
            y_pred_baseline_list.append(y_train.mean())
    ...
    horizon_results.append({
        ...
        'y_pred_direct': y_pred_direct.tolist(),
    })
...
# Remove predictions from saved results to keep file size reasonable
horizon_results_for_save = []
for hr in horizon_results:
    hr_copy = hr.copy()
    hr_copy.pop('y_test_indices', None)
    hr_copy.pop('y_pred_baseline', None)
    hr_copy.pop('y_pred_direct', None)
    horizon_results_for_save.append(hr_copy)
...
output_data = {
    ...
    'overall_horizon_results': horizon_results_for_save,
...
horizons = [h['horizon'] for h in horizon_results]
horizon_minutes = [h['horizon_minutes'] for h in horizon_results]
